...
async function getWorkoutByUserAndDate(res, email, date) {
    try {
        const aggregateResult =
            await sql.query`SELECT COUNT(*) AS workoutCount, SUM(duration) AS totalDuration FROM workout WHERE user_email = ${email} AND dayCreated = ${date}`
        const detailsResult =
            await sql.query`SELECT * FROM workout WHERE user_email = ${email} AND dayCreated = ${date}`
...
async function updateEmail(res, email, newEmail) {
    ...
    try {
        ...
        await transaction
            .request()
            .query(`ALTER TABLE workout NOCHECK CONSTRAINT FK_users`)
        // Update the email in the workout table first
        await transaction
            .request()
            .query(
                `UPDATE workout SET user_email = '${newEmail}' WHERE user_email = '${email}'`
            )
        ...
        await transaction
            .request()
            .query(`ALTER TABLE workout CHECK CONSTRAINT FK_users`)
